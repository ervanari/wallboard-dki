...
const AverageDuration: React.FC = () => {
  ...
  const acd = data?.acd || '00:00';
  const asa = data?.asa || '00:00';
  const aht = data?.aht || '00:00';
  const acw = data?.acw || '00:00';
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="flex flex-row justify-between items-center p-2">
            <div className="relative group">
              <div className="text-xs text-black dark:text-white mb-1">
                ASA
                <div className="absolute left-full top-1/2 transform -translate-y-1/2 ml-2 w-52 rounded bg-gray-800 dark:bg-gray-700 text-white text-[10px] p-2 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-50">
                  Average Speed of Answer merupakan rata-rata waktu yang dibutuhkan Agent dalam menjawab panggilan.
                </div>
              </div>
              <div className="text-xs text-black dark:text-white">{asa}</div>
            </div>
          ...
          <div className="flex flex-row justify-between items-center p-2">
            <div className="relative group">
              <div className="text-xs text-black dark:text-white mb-1">
                ACD
                <div className="absolute bottom-full -translate-x-1/2 w-52 rounded bg-gray-800 dark:bg-gray-700 text-white text-[10px] p-2 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-50">
              ...
              <div className="text-xs text-black dark:text-white">{acd}</div>
            </div>
          ...
          <div className="flex flex-row justify-between items-center p-2">
            <div className="relative group">
              ...
              <div className="text-xs text-black dark:text-white">{acw}</div>
            </div>
            <div className={`px-2 py-0.5 rounded-full bg-red-200`}>
          ...
          <div className="flex flex-row justify-between items-center p-2">
            <div className="relative group">
              <div className="text-xs text-black dark:text-white mb-1">
                AHT
                <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 w-52 rounded bg-gray-800 dark:bg-gray-700 text-white text-[10px] p-2 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-50">
                  Average Handling Time adalah rata-rata waktu yang dihabiskan Agent untuk menangani satu panggilan.
                </div>
              </div>
              <div className="text-xs text-black dark:text-white">{aht}</div>
            </div>
